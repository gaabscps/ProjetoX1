...
export default function UserSection({ profile, location, isVisiting }: UserSectionProps) {
  ChartJS.register(ArcElement, Tooltip, Legend)
  const isTablet = useTabletHook()
  const isMobile = useMobileHook()

  const [onHover, setOnHover] = useState(false)
  const [isFollowing, setIsFollowing] = useState(profile?.youFollow)
  ...
  useEffect(() => {
    setIsFollowing(profile?.youFollow)
  }, [profile])
    ...
    <Body>
      ...
      <div className='user-section-container'>
        ...
        >
          ...
          <div className={`levelStats ${onHover ? 'levelStatsHover' : 'levelStatsOff'}`}>
        ...
        <div className='ml-1 w-100 user-stats-container'>
          <div
            ...
            className='nationalityTag-container'
          >
            ...
            {isVisiting && (
              <div className='d-flex align-items-center' style={{ marginLeft: '25px' }}>
                ...
                <Button
                  ...
                  theme={isFollowing ? 'selected' : 'standard'}
                  margin='0 4px'
                  width='80px'
                  height='25px'
                  content={isFollowing ? 'Seguindo' : 'Seguir'}
                />
                {isFollowing && (
                  <p
                    onClick={() => handleFollow()}
          ...
          <div className='home-user-stats'>
            ...
            <span className='color-black-7'>
              {profile?.timesyouChangelled || '--'}x que desafiou
            </span>
            <span className='color-black-7'>
              {profile?.timesChangelled || '--'}x que foi desafiado
            </span>
            {(!isTablet || !isMobile) && <span className='color-black-7'>{'//'}</span>}
