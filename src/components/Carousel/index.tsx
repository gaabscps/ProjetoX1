...
export function Carrossel({ items, title, centerButton, isBanner }: CarouselProps) {
    ...
    <>
      ...
      <div style={{ width: '100%' }} className='d-flex carousel-align'>
        ...
        <div
          ...
          onTouchMove={isBanner ? handleTouchMoveCaptureBanner : handleTouchMove}
            ...
            >
              {items.map((item, index) => (
                <li key={index} className='carousel-item'>
                  {item ? <CarouselItem item={item} /> : <div className='empty-item' />}
                </li>
              ))}
            </ul>
          </div>
